from NightingaleORM.dbmodel import Model, useTransaction
...
def mian():
    ...
    dal = TaskModel()
    dal.addShow(TaskModel.tflag)
    dal.addShow(TaskModel.tupdatetime)
    dal.addJoin(TaskModel.tupdatetime == datetime.datetime.now())
    dal.addWhere(TaskModel.tid == 1)
    dal.addWhere(TaskModel.tupdatetime == datetime.datetime.now())
    dal = dal.startBrackets()
    dal.addWhere(TaskModel.tiid == 2)
    dal.addWhere("temployerid>0")
